...
export function BookingForm() {
  ...
  useEffect(() => {
    const stored = sessionStorage.getItem("aerolink:prefill-destination");
    if (!stored) return;
    sessionStorage.removeItem("aerolink:prefill-destination");
  ...
  useEffect(() => {
    ...
    window.addEventListener("aerolink:prefill-destination", onPrefill);
    return () =>
      window.removeEventListener("aerolink:prefill-destination", onPrefill);
  ...
  if (success) {
    const whatsappMessage = `Hi AeroLink Ghana! Booking ${success.reference}\nPrice: ${formatCurrency(success.price)}${success.paid ? " (PAID)" : ""}`;
    return (
      <div className="overflow-hidden border border-border bg-white shadow-xl">
        ...
        <div className="px-8 py-8 text-center">
          ...
          <p className="mt-4 text-sm text-muted">
            Confirmation sent to your email. Your chauffeur will contact you before pickup.
          </p>
                ...
                {(["PICKUP", "DROPOFF"] as const).map((type) => (
                  ...
                  >
                    ...
                    <span className="text-xs font-bold uppercase tracking-wider">
                      {type === "PICKUP" ? "Airport Pickup" : "Airport Drop-off"}
                    </span>
                  </button>
                ))}
              </div>
            ...
            <div className="space-y-6 animate-in fade-in">
              ...
              <div className="flex items-start gap-3 border border-border bg-light-blue-bg/50 p-4">
                ...
                <p className="text-xs leading-relaxed text-muted">
                  Your fare is fixed and guaranteed. Free cancellation up to 48 hours before pickup.
                  All prices include airport access, fuel, and gratuity.
                </p>
              </div>
            </div>
          )}
