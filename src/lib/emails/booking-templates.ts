...
function bookingDetailsTable(data: BookingEmailData): string {
  const rows = [
    ...
    ["Transfer", data.type === "PICKUP" ? "Airport Pickup" : "Airport Drop-off"],
...
export function customerBookingConfirmationHtml(
  ...
  const intro = paid
    ? `Hi ${data.customerName}, thank you — your payment was successful and your chauffeured transfer is confirmed.`
    : `Hi ${data.customerName}, thank you for choosing ${BRAND.name}. We've received your reservation and will confirm shortly.`;
  ...
  return layout(`
    ...
    <p style="margin:0 0 16px;font-size:14px;line-height:1.6;color:#64748b;">
      Your chauffeur will contact you on <strong>${data.customerPhone}</strong> before pickup.
...
export function adminNewBookingHtml(data: BookingEmailData): string {
  return layout(`
    <h1 style="margin:0 0 8px;font-size:22px;color:#1B365D;">New Booking Alert</h1>
    <p style="margin:0 0 24px;font-size:14px;color:#64748b;">
      A new transfer request has been submitted via the website.
    </p>
...
export function customerBookingConfirmationText(
  ...
  return `${paid ? "Booking Confirmed" : "Booking Received"} — ${data.reference}

Hi ${data.customerName},

${paid ? "Your payment was successful." : "We have received your reservation."}
